...
function postNetworkConfig(url, headers, data, noParse) {
    let params = ""
    if (data) {
        params = []
        for (const key in data) {
...
async function jdSign() {
    ...
    // 领取水滴
    result = await postNetworkConfig(`https://daojia.jd.com/client?_jdrandom=${Date.now()}`, {
        "referer": "https://daojia.jd.com/taroh5/h5dist/"
    }, {
        "functionId": "plantBeans/getWater",
        "city_id": "4",
        "body": { "activityId": "23b9682aea52934" },
        "deviceId": deviceId
    })

    $.log(`当前领取水滴${result.result.addWater}`);
    ...
    do {
        result = await postNetworkConfig(`https://daojia.jd.com/client?_jdrandom=${Date.now()}`, {
            ...
            "city_id": "4",
            "body": { "activityId": "23b96863bbc0b7a" },
            "deviceId": deviceId
        })
    ...
    let cur = result.result.cur;

    $.log(`水滴${cur.water},当前等级${cur.level},成长值${cur.levelProgress}/${cur.totalProgress}`);
    if (cur.water < 200) {
        $.log(`剩余水滴小于200，停止浇水`);
        return;
    }
    ...
    do {
        result = await postNetworkConfig(`https://daojia.jd.com/client?_jdrandom=${Date.now()}`, {
            ...
            "city_id": "4",
            "body": { "activityId": "23b9682aea52934", "waterAmount": 100 },
            "deviceId": deviceId
        })
        count++;
    } while (result.result.water > 200)

    $.log(`账号【${$.index}】${$.nickName}完成浇水,本次浇水${count}次`);
